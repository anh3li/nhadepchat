...
const getProduct = cache(async (slug:string) => getD1().prepare(`${productSelect()} WHERE p.slug=? AND p.status='approved' LIMIT 1`).bind(slug).first<Record<string,any>>());
...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <><ViewTracker productId={p.id}/><main className="subpage product-detail-page">
    ...
    <Suspense fallback={<p role="status">Đang tải bản vẽ liên quan…</p>}><RelatedProducts category={p.category} productId={p.id}/></Suspense>
  </main></>;
}

async function RelatedProducts({category, productId}:{category:string;productId:string}) {
  const related = await getD1().prepare(`${productSelect()} WHERE p.status='approved' AND p.category=? AND p.id<>? ORDER BY p.approved_at DESC LIMIT 4`).bind(category,productId).all<Record<string,any>>();
  return related.results.length ? <section className="detail-related"><div className="section-heading"><h2>BẢN VẼ LIÊN QUAN</h2></div><PublicProductGrid products={related.results}/></section> : null;
}
