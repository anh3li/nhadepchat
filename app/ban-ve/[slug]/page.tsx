...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <><MarketplaceHeader/><main className="subpage product-detail-page">
    ...
    <section className="detail-summary"><div className="detail-title-row"><p className="eyebrow">{p.category}{p.building_type&&p.building_type!==p.category?` · ${p.building_type}`:''}{p.style?` · ${p.style}`:''}</p><FavoriteButton productId={p.id} returnUrl={`/ban-ve/${p.slug}`} className="detail-favorite"/></div><h1>{p.title}</h1><a className="seller-line" href={`/kts/${p.seller_slug}`}><span className="tiny-avatar">{p.seller_name.split(' ').slice(-2).map((x:string)=>x[0]).join('')}</span><b>{p.seller_name}</b>{p.verification_status==='verified'&&<i>✓</i>}</a>{p.short_description&&<p className="detail-short">{p.short_description}</p>}<dl>{p.width&&p.length&&<div><dt>Kích thước</dt><dd>{p.width} × {p.length}m</dd></div>}{p.floors&&<div><dt>Số tầng</dt><dd>{p.floors}</dd></div>}{p.area&&<div><dt>Diện tích</dt><dd>{p.area}m²</dd></div>}{p.formats&&<div><dt>Định dạng</dt><dd>{p.formats}</dd></div>}</dl>{(disciplines.results.length>0||tools.results.length>0)&&<div className="disciplines">{disciplines.results.map(x=><span key={x.discipline}>✓ {x.discipline}</span>)}{tools.results.map(x=><span className="tool" key={x.tool}>{x.tool}</span>)}</div>}<div className="detail-purchase"><strong className={p.is_free?'detail-price free-price':'detail-price'}>{p.is_free?'MIỄN PHÍ':`${Number(p.price).toLocaleString('vi-VN')}đ`}</strong>{p.is_free&&firstFile?<a className="download" href={`/api/download/${firstFile.id}`}>TẢI MIỄN PHÍ</a>:<><button className="download" disabled>MUA HỒ SƠ</button><small>Thanh toán sẽ được mở trong phiên bản tiếp theo.</small></>}</div></section>
