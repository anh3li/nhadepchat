...
export default async function AccountPage() {
  ...
  return <DashboardShell name={session.user.name} admin={access.admin} seller={access.seller}><div className="account-workspace">
    ...
    <div className="account-page"><AccountPanel name={session.user.name} email={session.user.email} role={access.role} seller={access.seller} avatarUrl={access.avatarKey ? `/api/profile-avatar/${session.user.id}?v=${encodeURIComponent(access.avatarKey)}` : session.user.image || null} /></div>
  </div></DashboardShell>;
}
