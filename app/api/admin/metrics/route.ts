...
export async function GET(request: Request) {
  ...
  return Response.json({ settings: { ...defaults, ...saved }, products: products.results });
}
...
export async function PUT(request: Request) {
  ...
  const { homeUseReal, home, productUseReal, ratingUseReal, products } = parsed.data;
  const db = getD1();
  const now = Date.now();
  const statements = [
    db.prepare(`INSERT INTO metric_settings
      (id,home_use_real,home_product_count,home_free_count,home_seller_count,home_download_count,product_use_real,rating_use_real,updated_at)
      VALUES (1,?,?,?,?,?,?,?,?)
      ...
      rating_use_real=excluded.rating_use_real,updated_at=excluded.updated_at`)
      .bind(homeUseReal ? 1 : 0, home.productCount, home.freeCount, home.sellerCount, home.downloadCount, productUseReal ? 1 : 0, ratingUseReal ? 1 : 0, now),
