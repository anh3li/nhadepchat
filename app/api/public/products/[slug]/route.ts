...
export async function GET(_request:Request,{params}:{params:Promise<{slug:string}>}){
  ...
  const[assets,formats,disciplines,tools,keywords]=await Promise.all([
    db.prepare('SELECT id,type,sort_order FROM product_assets WHERE product_id=? ORDER BY sort_order').bind(product.id).all(),
  ...
  return Response.json({...product,assets:assets.results,formats:formats.results.map((x:any)=>x.format),disciplines:disciplines.results.map((x:any)=>x.discipline),tools:tools.results.map((x:any)=>x.tool),keywords:keywords.results.map((x:any)=>x.keyword)});
}
