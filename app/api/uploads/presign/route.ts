...
export async function POST(request: Request){
  ...
  const isPreview=kind==='preview'||kind==='thumbnail';
  const allowed=isPreview?['webp']:['dwg','skp','rvt','pdf','xlsx','docx','zip','rar'],max=isPreview?4*1024*1024:250*1024*1024;
  if(!allowed.includes(ext)||size<=0||size>max||!isAllowedMime(ext,mime,isPreview))return jsonError('Metadata upload không hợp lệ.',422);
  ...
  const bucketName=isPreview?(env.R2_PUBLIC_BUCKET||env.R2_BUCKET_NAME):(env.R2_PRIVATE_BUCKET||env.R2_BUCKET_NAME);
  if(!env.R2_ACCOUNT_ID||!bucketName||!env.R2_ACCESS_KEY_ID||!env.R2_SECRET_ACCESS_KEY)return Response.json({direct:false,uploadUrl:'/api/uploads'});
