...
function Collections({summaries}:{summaries:Record<string,CollectionSummary>}){return <section className="collections" id="collections"><div className="section-heading"><h2>BỘ SƯU TẬP NỔI BẬT</h2><Link href="/bo-suu-tap">Xem tất cả →</Link></div><div className="collection-grid">{collections.map(c=>{const summary=summaries[c.id],image=summary?.cover_id?`/api/assets/${summary.cover_id}`:'';return <Link className="collection-card" href={`/bo-suu-tap#${c.id}`} key={c.name}>{image?<SafeImage src={image} alt={c.name}/>:<span className="image-placeholder"/>}<span><strong>{c.name}</strong><small>{Number(summary?.count||0)} hồ sơ</small></span></Link>})}</div></section>}
