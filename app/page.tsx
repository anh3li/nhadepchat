...
function Header() {
  ...
  return <header className="site-header"><div className="shell header-inner">
    ...
    <nav className={mobile?'main-nav mobile-open':'main-nav'}>
      ...
      <a href="#sell">Đăng bán</a><a className="nav-cart" href="#cart"><span>♧</span> Giỏ hàng <i>0</i></a><a className="account-link" href="#account"><span>TK</span>Tài khoản</a>
    </nav><button className="menu-toggle" onClick={()=>setMobile(!mobile)} aria-label="Mở menu">☰</button>
  </div></header>;
}

function MegaMenu(){return <div className="mega-menu">{drawingMenu.map(group=><section key={group.title}><h3>{group.title}</h3>{group.links.map(link=><a href="#drawings" key={link}>{link}</a>)}</section>)}<a className="menu-view-all" href="#drawings">Xem tất cả bản vẽ →</a></div>}
...
function ProductCard({product,onOpen}:{product:typeof products[number];onOpen:()=>void}){return <article className="product-card" onClick={onOpen} tabIndex={0} onKeyDown={e=>(e.key==='Enter'||e.key===' ')&&onOpen()}><div className="product-image"><img src={product.image} alt={product.title} loading="lazy"/><b className={product.badge==='MIỄN PHÍ'?'card-badge free':'card-badge'}>{product.badge}</b><button className="save" aria-label="Lưu bản vẽ" onClick={e=>e.stopPropagation()}>♡</button></div><div className="product-body"><h3>{product.title}</h3><p className="product-type">{product.type}</p><p className="product-meta">{product.size} <i/> {product.floors}</p><div className="formats">{product.files.split(' · ').map(file=><span key={file}>{file}</span>)}</div><div className="author"><span className="tiny-avatar">{product.architect.split(' ').slice(-2).map(x=>x[0]).join('')}</span><span>{product.architect}<b>✓</b></span><strong className={product.price==='MIỄN PHÍ'?'free-price':''}>{product.price}</strong></div></div></article>}
