...
function Header() {
  ...
  return <header className="site-header"><div className="shell header-inner">
    ...
    <form className="search" onSubmit={e=>e.preventDefault()}><input aria-label="Tìm kiếm bản vẽ" placeholder="Tìm nhà 5x20, biệt thự 2 tầng, file CAD..."/><button aria-label="Tìm kiếm"><Search size={20} strokeWidth={1.8}/></button></form>
    <nav className={mobile?'main-nav mobile-open':'main-nav'}>
      <div className="nav-group" onMouseEnter={()=>!pinned&&setOpen('drawings')} onMouseLeave={()=>!pinned&&setOpen(null)}><button className={open==='drawings'?'open':''} onClick={()=>toggleMenu('drawings')}>Bản vẽ <ChevronDown className="nav-chevron" size={15} strokeWidth={1.8}/></button>{open==='drawings'&&<MegaMenu/>}</div>
      <a href="#collections">Bộ sưu tập</a>
      <div className="nav-group community" onMouseEnter={()=>!pinned&&setOpen('community')} onMouseLeave={()=>!pinned&&setOpen(null)}><button className={open==='community'?'open':''} onClick={()=>toggleMenu('community')}>Cộng đồng <ChevronDown className="nav-chevron" size={15} strokeWidth={1.8}/></button>{open==='community'&&<CommunityMenu/>}</div>
