...
export function ProductCard({product:p,preview=false,compact=false}:{product:ProductCardData;preview?:boolean;compact?:boolean}){
  ...
  return <article className={`product-card${compact?' compact':''}`}>{preview?<div className="product-card-link">{content}</div>:<Link className="product-card-link" href={`/ban-ve/${p.slug}`}>{content}</Link>}{!preview&&p.id&&<FavoriteButton productId={p.id} returnUrl={`/ban-ve/${p.slug}`}/>}</article>;
}
