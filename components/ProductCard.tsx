...
export function ProductCard({product:p,preview=false}:{product:ProductCardData;preview?:boolean}){
  ...
  const content=<><div className="product-image">{image?<img src={image} alt={p.title} loading="lazy"/>:<div className="image-placeholder"/>}<b className={isFree?'card-badge free':'card-badge'}>{isFree?'MIỄN PHÍ':'MỚI'}</b></div><div className="product-body"><h3>{p.title}</h3><p className="product-type">{p.category||p.type}</p><p className="product-meta">{dimensions}<i/>{floors}</p>{formats.length>0&&<div className="formats">{formats.map((format:string)=><span key={format}>{format}</span>)}</div>}<div className="card-proof"><span><Star size={13} fill={Number(p.review_count||0)?'currentColor':'none'}/>{p.review_count?Number(p.rating).toFixed(1):'Mới'}</span><span><Eye size={13}/>{Number(p.view_count||p.views||0).toLocaleString('vi-VN')}</span><span><Download size={13}/>{Number(p.download_count||p.downloads||0).toLocaleString('vi-VN')}</span></div><div className="author"><span className="tiny-avatar">{seller.split(' ').filter(Boolean).slice(-2).map((x:string)=>x[0]).join('').toUpperCase()}</span><span>{seller}{p.verification_status==='verified'&&<b>✓</b>}</span><strong className={isFree?'free-price':''}>{price}</strong></div></div></>;
  return <article className="product-card">{preview?<div className="product-card-link">{content}</div>:<Link className="product-card-link" href={`/ban-ve/${p.slug}`}>{content}</Link>}{!preview&&p.id&&<FavoriteButton productId={p.id} returnUrl={`/ban-ve/${p.slug}`}/>}</article>;
}
