...
export function ProductCard({product:p,preview=false,compact=false}:{product:ProductCardData;preview?:boolean;compact?:boolean}){
  const image=p.image||p.thumbnailUrl||previewImageUrl(p.cover_id,p.cover_key,p.thumbnail_key),isFree=Boolean(p.is_free)||p.priceLabel==='MIỄN PHÍ',formats=Array.isArray(p.formats)?p.formats:String(p.formats||p.files||'').split(/[·,]/).map((x:string)=>x.trim()).filter(Boolean),seller=p.seller_name||p.architect||'Nhà Đẹp Chất';
