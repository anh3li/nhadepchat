...
export function ImageViewer({images,initial,onIndex,onClose}:{images:GalleryImage[];initial:number;onIndex:(i:number)=>void;onClose:()=>void}){
  ...
  function go(delta:number){const next=(index+delta+images.length)%images.length;if(next===index)return;setIndex(next);onIndex(next);setLoading(true);setError(false);reset()}
