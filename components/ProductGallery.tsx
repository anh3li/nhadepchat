...
function Lightbox({images,initial,onIndex,onClose}:{images:GalleryImage[];initial:number;onIndex:(i:number)=>void;onClose:()=>void}){
  ...
  function go(delta:number){if(zoom>1)return;const next=(index+delta+images.length)%images.length;setIndex(next);onIndex(next);setLoading(true);setError(false);reset()}
  ...
  function down(event:PointerEvent){(event.currentTarget as HTMLElement).setPointerCapture(event.pointerId);pointers.current.set(event.pointerId,{x:event.clientX,y:event.clientY});const points=[...pointers.current.values()];origin.current={x:event.clientX,y:event.clientY,panX:pan.x,panY:pan.y,distance:points.length===2?Math.hypot(points[0].x-points[1].x,points[0].y-points[1].y):0,zoom}}
  ...
  function up(event:PointerEvent){const point=pointers.current.get(event.pointerId);if(point&&pointers.current.size===1&&zoom===1){const dx=event.clientX-origin.current.x,dy=event.clientY-origin.current.y;if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy))go(dx<0?1:-1)}pointers.current.delete(event.pointerId)}
  return <div ref={dialog} className="lightbox" role="dialog" aria-modal="true" aria-label={`Ảnh ${index+1} trên ${images.length}`} tabIndex={-1}>
    <div className="lightbox-toolbar"><button type="button" onClick={()=>scaleTo(zoom-.5)} disabled={zoom<=1} aria-label="Thu nhỏ" title="Thu nhỏ"><Minus/></button><span>{Math.round(zoom*100)}%</span><button type="button" onClick={()=>scaleTo(zoom+.5)} disabled={zoom>=4} aria-label="Phóng to" title="Phóng to"><Plus/></button><button type="button" onClick={reset} aria-label="Vừa màn hình" title="Vừa màn hình">Fit</button><button type="button" onClick={onClose} aria-label="Đóng" title="Đóng"><X/></button></div>
    {images.length>1&&<><button type="button" className="lightbox-prev" onClick={()=>go(-1)} disabled={zoom>1} aria-label="Ảnh trước" title="Ảnh trước"><ChevronLeft/></button><button type="button" className="lightbox-next" onClick={()=>go(1)} disabled={zoom>1} aria-label="Ảnh sau" title="Ảnh sau"><ChevronRight/></button></>}
    <div ref={stage} className={`lightbox-stage${zoom>1?' zoomed':''}`} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up} onWheel={event=>{event.preventDefault();scaleTo(zoom+(event.deltaY<0?.25:-.25))}} onDoubleClick={()=>scaleTo(zoom===1?2:1)}>{loading&&!error&&<span className="gallery-loading">Đang tải ảnh…</span>}{error?<span className="gallery-error">Không thể tải ảnh này.</span>:<img ref={image} src={active.src} alt={active.alt} draggable={false} onLoad={()=>setLoading(false)} onError={()=>{setLoading(false);setError(true)}} style={{transform:`translate3d(${pan.x}px,${pan.y}px,0) scale(${zoom})`}}/>}</div>
