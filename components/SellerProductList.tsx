...
export function SellerProductList({initial}:{initial:Product[]}){
  ...
  return <>{error&&<p className="form-error" role="alert">{error}</p>}<div className="seller-products">{products.map(product=><article key={product.id}><div className="seller-product-thumb" role={product.cover_id?'img':undefined} aria-label={product.cover_id?product.title:undefined} style={product.cover_id?{backgroundImage:`url("/api/assets/${product.cover_id}")`}:undefined}>{!product.cover_id&&<ImageIcon/>}</div><div className="seller-product-info"><div><span className={`status ${product.status}`}>{labels[product.status]||product.status}</span><span className="updated"><Clock3/>Cập nhật {new Date(product.updated_at).toLocaleDateString('vi-VN')}</span></div><h2>{product.title}</h2><p>{product.category} · {product.images} ảnh · {product.files} file · {product.views||0} lượt xem · {product.downloads||0} lượt tải · {product.review_count?`${Number(product.rating).toFixed(1)} sao`:'chưa đánh giá'}</p>{product.rejection_reason&&<div className="rejection-note"><b>Lý do từ chối</b>{product.rejection_reason}</div>}</div><strong className={product.is_free?'free-price':''}>{product.is_free?'MIỄN PHÍ':`${Number(product.price).toLocaleString('vi-VN')}đ`}</strong><div className="seller-product-actions">{product.status==='approved'&&<Link href={`/ban-ve/${product.slug}`}><Eye/>Xem chi tiết</Link>}{['draft','rejected'].includes(product.status)&&<><Link href={`/dashboard/dang-ban?id=${product.id}`}><Edit3/>Sửa hồ sơ</Link><button type="button" disabled={busy===product.id} onClick={()=>submit(product.id)}><FileUp/>{busy===product.id?'Đang gửi…':'Gửi duyệt'}</button></>}</div></article>)}</div></>;
}
