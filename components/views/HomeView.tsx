...
import { HomeDrawings } from '../HomeDrawings';
...
export function HomeView({data}:{data:any}) {
    ...
    <main className="shell marketplace home-reference">
      <section className="hero-main">
        <div className="hero-copy">
          <h1>Bản vẽ nhà đẹp,<br/>mẫu nhà đẹp</h1>
          ...
          <div className="hero-actions"><Link className="primary-btn" href="/tim-kiem">Tìm bản vẽ ngay</Link><Link className="secondary-btn" href="/tim-kiem?q=miễn phí">Bản vẽ miễn phí</Link></div>
      ...
      <aside className="popular-panel">
        ...
        {popular.length ? popular.slice(0, 5).map((product, index) => { const image=product.thumbnailUrl||previewImageUrl(product.cover_id,product.cover_key,product.thumbnail_key); const size=product.width&&product.length?`${product.width} × ${product.length}m`:'Chưa cập nhật'; return <Link className="popular-row" href={`/ban-ve/${product.slug}`} key={product.id}><span className="popular-rank">{index + 1}</span>{image?<SafeImage className="popular-media" src={image} alt={product.title}/>:<span className="image-placeholder popular-media"/>}<span className="popular-info"><strong>{product.title}</strong><small>{size} · {number(product.download_count)} lượt tải</small></span><b className={product.is_free?'free-price':''}>{product.is_free?'MIỄN PHÍ':`${number(product.price)}đ`}</b></Link> }) : <div className="panel-empty">Chưa có dữ liệu lượt tải.</div>}
      </aside>
      </div>
