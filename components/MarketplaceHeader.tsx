...
function AccountDropdown({viewer,logout}:{viewer:Viewer|null|undefined;logout:()=>void}){
  ...
  return <div className="account-menu"><header><b>{viewer.display_name}</b><span>{viewer.role==='admin'?'Quản trị viên':viewer.seller_id?'Người bán':'Thành viên'}</span></header><a href="/tai-khoan"><UserRound/>Tài khoản của tôi</a><a href="/tai-khoan/da-luu"><Heart/>Bản vẽ đã lưu</a><span className="menu-disabled"><PackageCheck/>Đã mua <small>Sắp mở</small></span>{viewer.seller_id&&<><a href="/dashboard"><LayoutDashboard/>Dashboard người bán</a><a href="/dashboard/san-pham"><FileText/>Sản phẩm của tôi</a><a href="/dashboard/dang-ban"><Store/>Đăng sản phẩm</a></>}{viewer.role==='admin'&&<a href="/admin/san-pham"><Settings/>Quản trị</a>}<button type="button" onClick={logout}><LogOut/>Đăng xuất</button></div>;
}
