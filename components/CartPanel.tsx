...
export function CartPanel(){
  ...
  return <div className="cart-layout"><section className="cart-items">{error&&<p className="form-error" role="alert">{error}</p>}{data?.items.map(item=><article key={item.id}>{item.cover_id?<SafeImage src={`/api/assets/${item.cover_id}`} alt={item.title} fallbackClassName="image-placeholder"/>:<span className="image-placeholder"/>}<div><Link href={`/ban-ve/${item.slug}`}>{item.title}</Link><p>{item.category} · {item.formats||'Đang cập nhật định dạng'}</p><small>{item.seller_name}</small></div><strong>{Number(item.price).toLocaleString('vi-VN')}đ</strong><button type="button" disabled={busy===item.id} onClick={()=>remove(item.id)} aria-label={`Xóa ${item.title}`}><Trash2/>{busy===item.id?'Đang xóa…':'Xóa'}</button></article>)}</section><aside className="cart-summary"><h2>Tóm tắt đơn hàng</h2><p><span>{data?.count||0} sản phẩm</span><b>{total.toLocaleString('vi-VN')}đ</b></p><div className="cart-payment-notice"><LockKeyhole/><span><b>Thanh toán trực tuyến đang được cấu hình</b>Giỏ hàng đã được lưu. Hệ thống sẽ chỉ mở thanh toán sau khi cổng thanh toán được xác minh an toàn.</span></div><button type="button" disabled>TIẾP TỤC THANH TOÁN</button><Link href="/tim-kiem">Tiếp tục xem bản vẽ</Link></aside></div>;
}
